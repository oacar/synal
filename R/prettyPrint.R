...
prettyPrint <- function (x, y, output = c("pdf", "tex", "dvi", "asis"), subset = NULL,
          file = NULL, alFile = NULL, askForOverwrite = TRUE, psFonts = FALSE,
          ...
          furtherCode = NA, verbose = FALSE)
{
  ...
  writeXStringSet(as(x, "XStringSet")[subset], filepath = alFile)
  if (verbose)
  ...
  if (length(toShow) == 1) {
    if (sum(width(toShow)) < width(x)[1])
      texOutput <- c(texOutput, paste("\\setends{consensus}{",
                                      start(toShow), "..", end(toShow), "}", sep = ""))
  }
  else {
    coList <- sapply(1:length(toShow), function(i) paste(start(toShow)[i],
                                                         "..", end(toShow)[i], sep = ""))
  ...
  if (output == "asis")
    ...
    if (output == "tex")
      ...
      tools::texi2dvi(texfile, quiet = !verbose, pdf = identical(output,
                                                          "pdf"), texinputs = system.file("tex", package = "msa"),
...
prettyPrintReadFile <- function (x, y, output = c("pdf", "tex", "dvi", "asis"), subset = NULL,
                         file = NULL, alFile = NULL, askForOverwrite = TRUE, psFonts = FALSE,
                         code = NA, paperWidth = 11, paperHeight = 8.5, margins = c(0.1,
                                                                                    0.3), shadingMode = c("identical", "similar", "functional"),
                         shadingModeArg = NA, shadingColors = c("blues", "reds", "greens",

...
analyze <- function(outputDirectory,orfName,types,best=T) {
  ...
  if(best){
    for(i in 2:length(types)){
      typeName <- types[i]
      dataTable <- analyzePairwise(typeName, outputDirectory, orfName,startAA, startDNA, stopAA, subalign, aa,df)
    ...
    for(i in 2:length(types)){
      ...
      for(j in 1:numOfOrfs){
        dataTable <- analyzePairwise(typeName, outputDirectory, orfName,startAA, startDNA, stopAA, subalign, aa,df,ids[j])
